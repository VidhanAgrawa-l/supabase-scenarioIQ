...
with open("firebase_improvement_feedback.json", "r") as f:
...
for user in users_data:
    ...
    user_data = json.loads(user["feedback_data"])  # Convert JSON string back to dict
    user_data = fix_datetime(user_data)  # Fix datetime issues

    # Insert into Supabase
    response = (
        supabase_client.table("improvement_feedback")
        .insert({"feedback_id": user_id, "feedback_data": user_data})
        .execute()
    )

...
with open("firebase_transcription.json", "r") as f:
...
for user in users_data:
    user_id = user["transcription_id"]
    user_data = json.loads(user["transcription_data"])  # Convert JSON string back to dict
    user_data = fix_datetime(user_data)  # Fix datetime issues

    # Insert into Supabase
    response = (
        supabase_client.table("transcription")
        .insert({"transcription_id": user_id, "transcription_data": user_data})
        .execute()
    )

...
with open("firebase_scenarios.json", "r") as f:
...
for user in users_data:
    user_id = user["scenarios_id"]
    user_data = json.loads(user["scenarios_data"])  # Convert JSON string back to dict
    user_data = fix_datetime(user_data)  # Fix datetime issues

    # Insert into Supabase
    response = (
        supabase_client.table("scenarios")
        .insert({"scenarios_id": user_id, "scenarios_data": user_data})
        .execute()
    )
